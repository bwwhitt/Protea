...
class batcache {
	...
	var $ignored_query_args = array(
		// 'hpt', 'eref', 'iref', 'fbid', 'om_rid', 'utm', 'utm_source', 'utm_content', 'utm_medium', 'utm_campaign', 'utm_term', 'fb_xd_bust', 'fb_xd_fragment', 'npt', 'module', 'iid', 'cid', 'icid', 'ncid', 'snapid', '_', 'fb_ref', 'fb_source',
		'fbclid', // New Facebook click tracker parameter.
	);
	...
	function ob($output) {
		...
		if ( $this->debug ) {
			...
			$this->headers['X-nananana'] = 'Batcache-Set';
		}
...
if ( ! empty( $_SERVER['HTTP_X_WP_NONCE'] ) || ! empty( $_SERVER['HTTP_X_HTTP_METHOD_OVERRIDE'] ) ) {
	return;
}
...
if (
	...
	// Add some debug info just before </head>
	if ( $batcache->debug ) {
		$batcache->headers['X-nananana'] = 'Batcache-Hit';
