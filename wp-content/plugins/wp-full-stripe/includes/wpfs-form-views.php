...
abstract class MM_WPFS_FormView implements MM_WPFS_FormViewConstants {
	...
	public function getFormHash() {
		...
		return esc_attr( MM_WPFS_Utils::generateFormHash(
			$formType,
			$formId,
			$this->form->name )
		);
	...
	protected function getSubmitButtonRawCaptionForForm($form ) {
	    ...
	    if ( $this instanceof MM_WPFS_CheckoutPaymentFormView ||
        ...
        } elseif ( $this instanceof MM_WPFS_InlinePaymentFormView ||
	...
	protected function prepareCountryOptions($selectedCountryCode ) {
        ...
        foreach (MM_WPFS_Countries::getAvailableCountries() as $countryKey => $country ) {
            ...
            if ( isset( $selectedCountryCode ) && $selectedCountryCode === $country['alpha-2'] ) {
        ...
        if ( false === $defaultCountrySelected ) {
            $selectCountryOption->setAttributes(
                array(
                    'disabled' => 'disabled',
                    'selected' => 'selected'
    ...
    protected function prepareStateOptions( $selectedStateCode ) {
        ...
        $selectStateOption = MM_WPFS_ControlUtils::createControl( $this->formHash, null, null,
            /* translators: Option label for the default option of the country selector dropdown */
            __( 'Select state', 'wp-full-stripe' ), null, null );
    ...
    protected function prepareAddresses() {
		...
		if ( $this->showAnyAddress() ) {
            $countryOptions = $this->prepareCountryOptions( $this->form->defaultBillingCountry );
		}
		if ( isset( $this->form->showAddress ) && 1 == $this->form->showAddress ) {
            ...
            $this->billingAddressStateSelect = MM_WPFS_ControlUtils::createControl( $this->formHash, self::FIELD_BILLING_ADDRESS_STATE_SELECT, null, null,
                __( 'State', 'wp-full-stripe' ), null );
            $this->billingAddressStateSelect->setOptions( $this->prepareStateOptions( null ));
		...
		if ( isset( $this->form->showShippingAddress ) && 1 == $this->form->showShippingAddress ) {
			...
			$this->shippingAddressState   = MM_WPFS_ControlUtils::createControl( $this->formHash, self::FIELD_SHIPPING_ADDRESS_STATE, null, null,
				...
				__( 'State', 'wp-full-stripe' ), null );
            $this->shippingAddressStateSelect = MM_WPFS_ControlUtils::createControl( $this->formHash, self::FIELD_SHIPPING_ADDRESS_STATE_SELECT, null, null,
                __( 'State', 'wp-full-stripe' ), null );
            $this->shippingAddressStateSelect->setOptions( $this->prepareStateOptions( null ));
...
abstract class MM_WPFS_SubscriptionFormView extends MM_WPFS_FormView implements MM_WPFS_SubscriptionFormViewConstants {
    ...
    protected function prepareProductPricing() {
        if ( MM_WPFS_Utils::getFormType( $this->form ) === MM_WPFS::FORM_TYPE_INLINE_SUBSCRIPTION ||
            MM_WPFS_Utils::getFormType( $this->form ) === MM_WPFS::FORM_TYPE_CHECKOUT_SUBSCRIPTION ) {
            ...
            $pricingData->couponCode = null;
            $pricingData->couponPercentOff = true;
            $pricingData->customAmount = null;
            $pricingData->quantity = 1;

            $this->productPricing = MM_WPFS_Pricing::createFormPriceCalculator( $pricingData )->getProductPrices();
        } else {
            $this->productPricing = [];
        }
    }
		...
		if ( ! is_null( $firstStripePlan ) ) {
			...
			$this->firstPlan->setAttributes(
				array(
					...
					self::ATTR_DATA_WPFS_VAT_PERCENT                                => $this->__currentVATPercent,
					self::ATTR_DATA_WPFS_PLAN_NAME                                  => MM_WPFS_Localization::translateLabel( $planName ),
					self::ATTR_DATA_WPFS_INTERVAL                                   => $planInterval,
					self::ATTR_DATA_WPFS_INTERVAL_COUNT                             => $planIntervalCount,
...
trait MM_WPFS_FormView_PricingAddOn {
    ...
    protected function getTaxLabel() {
        ...
        if ( $this->form->vatRateType !== MM_WPFS::FIELD_VALUE_TAX_RATE_NO_TAX ) {
...
trait MM_WPFS_FormView_InlineTaxAddOn {
    /** @var MM_WPFS_Control */
    ...
    protected function initInlineTaxFields() {
        ...
        $this->prepareTaxCountryAndState();
    }

    protected function prepareTaxCountryAndState() {
        $this->taxCountry = MM_WPFS_ControlUtils::createControl( $this->formHash, self::FIELD_TAX_COUNTRY, null, null,
            __( 'Country', 'wp-full-stripe' ), null );
        $this->taxCountry->setOptions( $this->prepareCountryOptions( $this->getDefaultCountry() ));

        $this->taxState = MM_WPFS_ControlUtils::createControl( $this->formHash, self::FIELD_TAX_STATE, null, null,
            __( 'State', 'wp-full-stripe' ), null );
        $this->taxState->setOptions( $this->prepareStateOptions( null ));
    }

    protected function prepareBuyingAsBusiness() {
        $this->buyingAsBusiness = MM_WPFS_ControlUtils::createControl( $this->formHash, self::FIELD_BUYING_AS_BUSINESS, null, null,
            __("I'm buying as a business", 'wp-full-stripe'), null );

        $this->businessName  = MM_WPFS_ControlUtils::createControl( $this->formHash, self::FIELD_BUSINESS_NAME, null, null,
            __( 'Business name', 'wp-full-stripe' ), null );
        $this->taxId         = MM_WPFS_ControlUtils::createControl( $this->formHash, self::FIELD_TAX_ID, null, null,
            __( 'Tax ID', 'wp-full-stripe' ), null );
    }
...
abstract class MM_WPFS_PaymentFormView extends MM_WPFS_FormView implements MM_WPFS_PaymentFormViewConstants {
	...
	protected function prepareProductPricing() {
	    if ( MM_WPFS_Utils::getFormType( $this->form ) === MM_WPFS::FORM_TYPE_INLINE_PAYMENT ||
             MM_WPFS_Utils::getFormType( $this->form ) === MM_WPFS::FORM_TYPE_CHECKOUT_PAYMENT ) {
	        $pricingData = new \StdClass;
            $pricingData->formType = MM_WPFS_Utils::getFormType( $this->form );
            $pricingData->formId = $this->form->name;
            $pricingData->country = $this->getDefaultCountry();
            ...
            $pricingData->couponCode = null;
            $pricingData->couponPercentOff = true;
            $pricingData->customAmount = ( MM_WPFS::PAYMENT_TYPE_CUSTOM_AMOUNT == $this->form->customAmount ) ? 0 : null;
            $pricingData->quantity = 1;
	...
	protected function prepareCustomAmountAndOptions() {
		...
		if ( MM_WPFS::PAYMENT_TYPE_CUSTOM_AMOUNT == $this->form->customAmount ) {
			...
			$this->customAmountOptions = MM_WPFS_ControlUtils::createControl( $this->formHash, self::FIELD_CUSTOM_AMOUNT, null, null,
				...
				__( 'Product', 'wp-full-stripe' ), null );
			...
			if ( count( $listOfProducts ) == 1 && $this->form->allowListOfAmountsCustom == 0 ) {
			...
			foreach ( $listOfProducts as $product ) {
				...
				$descriptionLabel = MM_WPFS_Localization::translateLabel( $description ) . ' - ' . $amountLabel . ( ! empty( $taxLabel ) ? ( ' + ' . $taxLabel ) : '' );
				...
				$formattedAmount    = MM_WPFS_Currencies::formatAmount( $this->currencyCode, $amount );
				...
				$optionAttributes = array(
					...
					self::ATTR_DATA_WPFS_PRODUCT_NAME                       => MM_WPFS_Localization::translateLabel( $description ),
				);
			...
			if ( 1 == $this->form->allowListOfAmountsCustom ) {
				$this->customAmount     = MM_WPFS_ControlUtils::createControl( $this->formHash, self::FIELD_CUSTOM_AMOUNT_UNIQUE, null, null,
					/* translators: Form field label for the payment amount that can be entered manually (think paying invoices) */
					__( 'Amount', 'wp-full-stripe' ), null );
				$customAmountAttributes = array_merge( $attributes, array( 'disabled' => true, self::ATTR_DATA_WPFS_AMOUNT_PRICE_ID => 'customAmount' ) );
...
class MM_WPFS_InlinePaymentFormView extends MM_WPFS_PaymentFormView implements MM_WPFS_FormView_InlineTaxAddOnConstants {
    ...
    use MM_WPFS_FormView_InlineTaxAddOn;
...
abstract class MM_WPFS_DonationFormView extends MM_WPFS_FormView implements MM_WPFS_DonationFormViewConstants {
    ...
    protected function prepareDonationAmountOptions() {
        ...
        if ( 1 == $this->form->allowCustomDonationAmount ) {
            ...
            if ( count( $donationAmountOptions ) > 0 ) {
...
class MM_WPFS_InlineSaveCardFormView extends MM_WPFS_InlinePaymentFormView {
...
class MM_WPFS_CheckoutSaveCardFormView extends MM_WPFS_CheckoutPaymentFormView {
