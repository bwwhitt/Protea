...
class MM_WPFS_Database {
     ...
     * @throws Exception
     */
	public function insertPayment( $paymentFormModel, $transactionData ) {
		global $wpdb;
	...
	public function getExistingStripeCustomersByEmail($email, $livemode ) {
        ...
        $donors      = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}fullstripe_donations WHERE email=%s AND livemode=%s GROUP BY StripeCustomerID;", $email, $livemode ? '1' : '0' ), ARRAY_A );
	 ...
	 * @throws Exception
	 */
	public function insertPopupFormSubmit($hash, $formHash, $formType, $referrer, $postData, $liveMode ) {
		global $wpdb;
