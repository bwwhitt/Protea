...
abstract class MM_WPFS_Public_FormModel implements MM_WPFS_Binder {
	...
	public function __construct() {
		...
		$this->__stripe = new MM_WPFS_Stripe( MM_WPFS::getStripeAuthenticationToken() );
	}
	...
	protected function prepareStripeCoupon() {
		if ( isset( $this->couponCode ) && ! empty( $this->couponCode ) ) {
			$this->__stripeCoupon = $this->retrieveCouponOrPromotionalCode();
			$this->couponId = $this->__stripeCoupon->id;
		}
	}
	...
	public function extractFormModelDataFromCheckoutSession( $checkoutSession ) {
		...
		if ( $checkoutSession instanceof \StripeWPFS\Checkout\Session ) {
			if ( isset( $checkoutSession->line_items ) && $checkoutSession->line_items->count() > 0 ) {
				$lineItem = $checkoutSession->line_items->first();
				if ( $lineItem instanceof \StripeWPFS\LineItem ) {
					if ( isset( $lineItem->discounts ) && count( $lineItem->discounts ) > 0 ) {
						$stripeDiscountObject = $lineItem->discounts[0];
						if ( $stripeDiscountObject instanceof \StripeWPFS\StripeObject && isset( $stripeDiscountObject->discount ) ) {
							$stripeDiscount = $stripeDiscountObject->discount;
							if ( $stripeDiscount instanceof \StripeWPFS\Discount ) {
	...
	/**
	 * @param \StripeWPFS\Coupon $_stripeCoupon
	 */
	public function setStripeCoupon( $_stripeCoupon ) {
		$this->__stripeCoupon = $_stripeCoupon;
	}
	...
	protected function retrieveCouponOrPromotionalCode() {
		$coupon          = null;
		$promotionalCode = $this->retrievePromotionalCode();
		if ( ! is_null( $promotionalCode ) ) {
			if ( false == $promotionalCode->active ) {
				$coupon = $this->retrieveCoupon();
			} else {
				$coupon = $promotionalCode->coupon;
			}
		} else {
			$coupon = $this->retrieveCoupon();
		}
...
abstract class MM_WPFS_Public_PaymentFormModel extends MM_WPFS_Public_FormModel {
	...
	protected function prepareAmountAndProductName() {
        ...
        if ( !$customAmount && MM_WPFS::PAYMENT_TYPE_CARD_CAPTURE !== $this->__form->customAmount ) {
            $products = json_decode( $this->__form->decoratedProducts );
            ...
            $this->__productName = $this->__form->productDesc;
            $this->__priceId = null;
            $this->__price   = null;
        }
	}
...
class MM_WPFS_Public_InlineSubscriptionFormModel extends MM_WPFS_Public_SubscriptionFormModel implements MM_WPFS_Public_InlineForm {
    ...
    protected function getFormFromDatabase() {
        return $this->__dao->getInlineSubscriptionFormByName( $this->getFormName() );
    }
}
...
class MM_WPFS_Public_CheckoutSubscriptionFormModel extends MM_WPFS_Public_SubscriptionFormModel implements MM_WPFS_Public_PopupForm {
    ...
    protected function getFormFromDatabase() {
        return $this->__dao->getCheckoutSubscriptionFormByName( $this->getFormName() );
    }
