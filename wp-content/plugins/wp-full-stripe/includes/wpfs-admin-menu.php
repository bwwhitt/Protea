...
class MM_WPFS_Admin_Menu {
	...
	public function __construct() {
		...
		$this->helpService = new MM_WPFS_HelpService();
    ...
    protected function prepareOnetimeProducts( $form ) {
        ...
        try {
            ...
            MM_WPFS_Utils::log( __FUNCTION__ . '(): Cannot retrieve Stripe one-time prices - ' . $ex->getMessage() );
        }
    ...
    protected function prepareTaxRates( $form ) {
        ...
        try {
            ...
            MM_WPFS_Utils::log( __FUNCTION__ . '(): Cannot retrieve Stripe tax rates - ' . $ex->getMessage() );
        }
	...
	function menu_pages() {
        ...
        $page_title =
            ...
            __( 'Full Stripe - Transactions', 'wp-full-stripe-admin' );
        $menu_title =
            /* translators: Menu title of the plugin in Wordpress admin */
            __( 'Full Stripe', 'wp-full-stripe-admin' );
        ...
        $submenu_page_title =
            ...
            __( 'Full Stripe - Manage forms', 'wp-full-stripe-admin' );
        $submenu_title      =
        ...
        $submenu_page_title =
            ...
            __( 'Full Stripe - Create form', 'wp-full-stripe-admin' );
        $submenu_title      =
        ...
        $submenu_page_title =
            ...
            __( 'Full Stripe - Settings', 'wp-full-stripe-admin' );
        $submenu_title      =
        ...
        $submenu_page_title =
            ...
            __( 'Full Stripe - Configure Stripe account', 'wp-full-stripe-admin' );
        $submenu_title      =
        ...
        $submenu_page_title =
            ...
            __( 'Full Stripe - Customize forms', 'wp-full-stripe-admin' );
        $submenu_title      =
        ...
        $submenu_page_title =
            ...
            __( 'Full Stripe - Email notifications', 'wp-full-stripe-admin' );
        $submenu_title      =
        ...
        $submenu_page_title =
            ...
            __( 'Full Stripe - Security settings', 'wp-full-stripe-admin' );
        $submenu_title      =
        ...
        $submenu_page_title =
            ...
            __( 'Full Stripe - Customer portal', 'wp-full-stripe-admin' );
        $submenu_title      =
        ...
        $submenu_page_title =
            ...
            __( 'Full Stripe - Wordpress dashboard settings', 'wp-full-stripe-admin' );
        $submenu_title      =
        ...
        $submenu_page_title =
            ...
            __( 'Full Stripe - Edit form', 'wp-full-stripe-admin' );
        $submenu_title      =
	...
	function compileFrontendAdminOptions() {
	    ...
	    $options = array(
            'customInputFieldCount' => MM_WPFS::getCustomFieldMaxCount(),
    ...
    public static function createMacroData() {
        ...
        $data->descriptions = apply_filters( 'fullstripe_macro_descriptions',
	...
	function fullstripe_admin_scripts() {
        ...
        $localizer = MM_WPFS_AdminScriptLocalizerFactory::createLocalizer( $page, $tab );
	...
	protected function getFormattedTimeDescription( $currentTime, $time ) {
        ...
        if ( $currentTime - $time < MINUTE_IN_SECONDS ) {
            ...
            $timeDescription = sprintf(
                ...
                */
                _n( 'Last event one second ago', 'Last event %d seconds ago', $seconds, 'wp-full-stripe-admin' ), $seconds );
            ...
            $timeDescription = sprintf(
                ...
                */
                _n( 'Last event one minute ago', 'Last event %d minutes ago', $minutes, 'wp-full-stripe-admin' ), $minutes );
            ...
            $timeDescription = sprintf(
                /* translators: Singular and plural message for time number of hours passed since the last webhook event
                   p1: number of hours
                */
                _n( 'Last event one hour ago', 'Last event %d hours ago', $hours, 'wp-full-stripe-admin' ), $hours );
        } elseif ( $currentTime - $time >= DAY_IN_SECONDS ) {
            ...
            $timeDescription = sprintf(
                /* translators: Singular and plural message for time number of days passed since the last webhook event
                   p1: number of days
                */
                _n( 'Last event one day ago', 'Last event %d days ago', $days, 'wp-full-stripe-admin' ), $days );
        }
	...
	protected function getStripeAccountData() {
        ...
        if ( MM_WPFS_Utils::isDemoMode() ) {
            ...
            $stripeData->testSecretKey      = $options[ MM_WPFS::OPTION_API_TEST_SECRET_KEY ];
            $stripeData->testPublishableKey = $options[ MM_WPFS::OPTION_API_TEST_PUBLISHABLE_KEY ];
            $stripeData->liveSecretKey      = $options[ MM_WPFS::OPTION_API_LIVE_SECRET_KEY ];
            $stripeData->livePublishableKey = $options[ MM_WPFS::OPTION_API_LIVE_PUBLISHABLE_KEY ];
            $stripeData->apiMode            = $options[ MM_WPFS::OPTION_API_MODE ];
            $stripeData->webHookUrl         = esc_attr( MM_WPFS_EventHandler::getWebhookEndpointURL() );
            $stripeData->webHookUrlLegacy   = esc_attr( MM_WPFS_EventHandler::getLegacyWebhookEndpointURL() );

            list( $liveStyle, $liveTitle, $liveDescription ) = $this->getLastWebHookEventDescription( $currentTime, $options[ MM_WPFS::OPTION_LAST_WEBHOOK_EVENT_LIVE ], true );
            $stripeData->liveEventStyle         = $liveStyle;
            $stripeData->liveEventTitle         = $liveTitle;
            $stripeData->liveEventDescription   = $liveDescription;

            list( $testStyle, $testTitle, $testDescription ) = $this->getLastWebHookEventDescription( $currentTime, $options[ MM_WPFS::OPTION_LAST_WEBHOOK_EVENT_TEST ], false );
            $stripeData->testEventStyle         = $testStyle;
            $stripeData->testEventTitle         = $testTitle;
            $stripeData->testEventDescription   = $testDescription;
        }
    ...
    protected function getStripeStatus() {
        ...
        if ( MM_WPFS_Utils::isDemoMode() ) {
            ...
            $options = get_option( 'fullstripe_options' );
            $stripeStatus->apiModeLabel = MM_WPFS_Admin::getApiModeLabelFromString( $options[ MM_WPFS::OPTION_API_MODE ] );
        }
    ...
    protected function getSecurityData() {
        ...
        $securityData->secureInlineForms     = $options[ MM_WPFS::OPTION_SECURE_INLINE_FORMS_WITH_GOOGLE_RE_CAPTCHA ];
        $securityData->secureCheckoutForms   = $options[ MM_WPFS::OPTION_SECURE_CHECKOUT_FORMS_WITH_GOOGLE_RE_CAPTCHA ];
        $securityData->secureCustomerPortal  = $options[ MM_WPFS::OPTION_SECURE_SUBSCRIPTION_UPDATE_WITH_GOOGLE_RE_CAPTCHA ];
        $securityData->fillInEmailForUsers   = $options[ MM_WPFS::OPTION_FILL_IN_EMAIL_FOR_LOGGED_IN_USERS ];
        ...
        if ( MM_WPFS_Utils::isDemoMode() ) {
            ...
            $securityData->recaptchaSecretKey    = $options[ MM_WPFS::OPTION_GOOGLE_RE_CAPTCHA_SECRET_KEY ];
            $securityData->recaptchaSiteKey      = $options[ MM_WPFS::OPTION_GOOGLE_RE_CAPTCHA_SITE_KEY ];
        }
     ...
     * @return stdClass
     */
    protected function getMyAccountData() {
        $myAccountData  = new \stdClass;
        $options        = get_option( 'fullstripe_options' );

        $myAccountData->cancelSubscriptions       = $options[MM_WPFS::OPTION_CUSTOMER_PORTAL_LET_SUBSCRIBERS_CANCEL_SUBSCRIPTIONS];
        $myAccountData->whenCancelSubscriptions   = $options[MM_WPFS::OPTION_CUSTOMER_PORTAL_WHEN_CANCEL_SUBSCRIPTIONS];
        $myAccountData->updowngradeSubscriptions  = $options[MM_WPFS::OPTION_CUSTOMER_PORTAL_LET_SUBSCRIBERS_UPDOWNGRADE_SUBSCRIPTIONS];
        $myAccountData->showInvoices              = $options[MM_WPFS::OPTION_CUSTOMER_PORTAL_SHOW_INVOICES_SECTION];
    ...
    function fullstripe_settings_my_account() {
	    ...
	    $myAccountData = $this->getMyAccountData();
	    $help          = $this->helpService->getContextSensitiveHelp( $_REQUEST );
    ...
    protected function getWordpressDasboardData() {
        $wpDashboardData  = new \stdClass;
        $options        = get_option( 'fullstripe_options' );

        $wpDashboardData->decimalSeparator                  = $options[ MM_WPFS::OPTION_DECIMAL_SEPARATOR_SYMBOL ];
        $wpDashboardData->useSymbolNotCode                  = $options[ MM_WPFS::OPTION_SHOW_CURRENCY_SYMBOL_INSTEAD_OF_CODE ];
        $wpDashboardData->currencySymbolAtFirstPosition     = $options[ MM_WPFS::OPTION_SHOW_CURRENCY_SIGN_AT_FIRST_POSITION ];
        $wpDashboardData->putSpaceBetweenSymbolAndAmount    = $options[ MM_WPFS::OPTION_PUT_WHITESPACE_BETWEEN_CURRENCY_AND_AMOUNT ];

        return $wpDashboardData;
    }
    ...
    private function formatTimestamp( $timestamp ) {
        return is_null( $timestamp ) ?
            ...
            date( 'F jS Y H:i', strtotime( $timestamp ) );
    }
	...
	private function getForms() {
        ...
        for ( $idx = 0; $idx < count( $forms ); $idx++ ) {
            ...
            $form->shortCode     = MM_WPFS_Utils::createShortCodeByTypeAndLayout( $form );
        ...
        if ( empty( $textFilter ) && $modeFilter === self::PARAM_VALUE_API_MODE_ALL ) {
            ...
            $forms = array_filter( $allForms, function( $form, $key ) use ( $textFilter, $modeFilter ) {
                ...
                if ( !empty( $textFilter )) {
                    ...
                    if ( strpos( strtolower( $form->displayName ), $lowerCaseTextFilter ) === false &&
        ...
        switch ( $tabId ) {
            case MM_WPFS_Admin_Menu::PARAM_VALUE_TAB_PAYMENTS: {
                ...
                if ( $transactionCount > 0 ) {
                    $transactionTable = new WPFS_OneTimePayments_Table();
            ...
            case MM_WPFS_Admin_Menu::PARAM_VALUE_TAB_SUBSCRIPTIONS: {
                ...
                if ( $transactionCount > 0 ) {
                    $transactionTable = new WPFS_Subscriptions_Table();
                    $transactionTable->prepare_items();
                }
            ...
            case MM_WPFS_Admin_Menu::PARAM_VALUE_TAB_DONATIONS: {
                ...
                if ( $transactionCount > 0 ) {
                    $transactionTable = new WPFS_Donations_Table();
                    $transactionTable->prepare_items();
                }
            ...
            case MM_WPFS_Admin_Menu::PARAM_VALUE_TAB_SAVED_CARDS: {
                ...
                if ( $transactionCount > 0 ) {
                    $transactionTable = new WPFS_SavedCards_Table();
                    $transactionTable->prepare_items();
                }
    ...
    protected function getEmailOptionsData() {
        ...
        $emailOptionsData->senderEmail = $options[ MM_WPFS::OPTION_EMAIL_NOTIFICATION_SENDER_ADDRESS ];
        $emailOptionsData->bccEmails   = json_decode( $options[ MM_WPFS::OPTION_EMAIL_NOTIFICATION_BCC_ADDRESSES ] );
    ...
    protected function getFormsOptionsData() {
        $formsOptionsData  = new \stdClass;
        $options           = get_option( 'fullstripe_options' );

        $formsOptionsData->fillInEmailForUsers = $options[ MM_WPFS::OPTION_FILL_IN_EMAIL_FOR_LOGGED_IN_USERS ];

        return $formsOptionsData;
    }
    ...
    protected function getFormsAppearanceData() {
        $formsAppearanceData  = new \stdClass;
        $options              = get_option( 'fullstripe_options' );

        $formsAppearanceData->customCss = $options[ MM_WPFS::OPTION_FORM_CUSTOM_CSS ];

        return $formsAppearanceData;
    }
     ...
     * @return array
     */
    protected function getPagesAndPosts( $form ) : array {
        $result = array();
        ...
        $query = new WP_Query( array(
            ...
            'post_type' => apply_filters( 'fullstripe_thank_you_post_types', array( 'page' ), $filterParams )
        ) );
     ...
     */
    protected function extractEmailTemplates( $formType, $emailTemplatesJson ) {
    ...
    protected function prepareEmailTemplates( $formType, $form ) {
        $templates           = MM_WPFS_Mailer::extractEmailTemplates( $formType, $form->emailTemplates );
    ...
    protected function prepareInlineSaveCardFormData( $form ) : \StdClass {
        ...
        $data->thankYouPages         = $this->getPagesAndPosts( $form );
        ...
        $data->customFieldMaxCount   = MM_WPFS::getCustomFieldMaxCount();
    ...
    protected function prepareRecurringProducts( $form ) {
        ...
        try {
            ...
            MM_WPFS_Utils::log( __FUNCTION__ . '(): Cannot retrieve Stripe recurring prices - ' . $ex->getMessage() );
        }
        foreach ($stripePrices as $stripePrice) {
            if (array_search($stripePrice->id, $priceIds) !== false) {
	...
	private function get_settings_nav_tab_items() {
        ...
        $item_customer_portal = array(
            'tab'     => 'customer-portal',
            'caption' => __( 'Customer Portal', 'wp-full-stripe-admin' ),
            'content' => MM_WPFS_Assets::templates( 'admin/partials/settings_tab_customer_portal.php' )
        );
		$item_logs          = array(
			'tab'     => 'logs',
		...
		array_push( $nav_tab_items, $item_security );
        array_push( $nav_tab_items, $item_customer_portal );
...
class MM_WPFS_AdminScriptLocalizerFactory {
     ...
     */
    public static function createLocalizer( $page, $tab ) : MM_WPFS_AdminScriptLocalizer {
        switch ( $page ) {
            case MM_WPFS_Admin_Menu::SLUG_SETTINGS_STRIPE:
                return new MM_WPFS_SettingsStripeLocalizer( $tab );

            case MM_WPFS_Admin_Menu::SLUG_SETTINGS_FORMS:
                return new MM_WPFS_SettingsFormsLocalizer( $tab );

            case MM_WPFS_Admin_Menu::SLUG_SETTINGS_CUSTOMER_PORTAL:
                return new MM_WPFS_SettingsCustomerPortalLocalizer( $tab );

            case MM_WPFS_Admin_Menu::SLUG_SETTINGS_SECURITY:
                return new MM_WPFS_SettingsSecurityLocalizer( $tab );

            case MM_WPFS_Admin_Menu::SLUG_SETTINGS_EMAIL_NOTIFICATIONS:
                return new MM_WPFS_SettingsEmailNotificationsLocalizer( $tab );

            case MM_WPFS_Admin_Menu::SLUG_CREATE_FORM:
                return new MM_WPFS_CreateFormLocalizer( $tab );

            case MM_WPFS_Admin_Menu::SLUG_EDIT_FORM:
                return new MM_WPFS_EditFormLocalizer( $tab, $_GET );

            case MM_WPFS_Admin_Menu::SLUG_FORMS:
                return new MM_WPFS_FormsLocalizer( $tab );

            case MM_WPFS_Admin_Menu::SLUG_TRANSACTIONS:
                return new MM_WPFS_TransactionsLocalizer( $tab );

            default:
                return new MM_WPFS_DefaultLocalizer( $tab );
        }
    }
}
...
abstract class MM_WPFS_AdminScriptLocalizer {
    ...
    public function __construct( $tab = null, $getParameters = null ) {
        $this->tab = $tab;
...
class MM_WPFS_SettingsEmailNotificationsLocalizer extends MM_WPFS_AdminScriptLocalizer {
        ...
        if (
            ...
            wp_localize_script( 'wp-full-stripe-admin-js', 'wpfsAdminL10n', array (
                ...
                'resetTemplateConfirmationMessage'   =>
                    /* translators: Confirmation message displayed  'Reset email template' dialog */
    ...
    public function getSettingsOptions() {
        ...
        if ( $this->tab === MM_WPFS_Admin_Menu::PARAM_VALUE_TAB_TEMPLATES ) {
            $wpOptions = get_option( 'fullstripe_options' );

            $options['emailTemplates'] = json_decode( $wpOptions[ MM_WPFS::OPTION_EMAIL_TEMPLATES ] );
            $options['defaultEmailTemplates'] = MM_WPFS_Mailer::getDefaultEmailTemplates();
            $options['macros']  = MM_WPFS_Admin_Menu::createMacroData();
        }
...
class MM_WPFS_EditFormLocalizer extends MM_WPFS_AdminScriptLocalizer {
    ...
    public function __construct( $tab = null, $getParameters = null ) {
        parent::__construct( $tab, $getParameters );
