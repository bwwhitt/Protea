...
class MM_WPFS_Admin {
    ...
    public static function formatIntervalLabelAdmin( $interval, $intervalCount ) {
		...
		if ( $interval === "year" ) {
			$intervalLabel = sprintf(
                 ...
                 * p1: interval count
                 */
                _n( 'year', '%d years', $intervalCount, 'wp-full-stripe-admin' ), number_format_i18n( $intervalCount ) );
		} elseif ( $interval === "month" ) {
			$intervalLabel = sprintf(
                /* translators: Singular and plural monthly interval label
                 * p1: interval count
                 */
			    _n( 'month', '%d months', $intervalCount, 'wp-full-stripe-admin' ), number_format_i18n( $intervalCount ) );
		} elseif ( $interval === "week" ) {
			$intervalLabel = sprintf(
                /* translators: Singular and plural weekly interval label
                 * p1: interval count
                 */
			    _n( 'week', '%d weeks', $intervalCount, 'wp-full-stripe-admin' ), number_format_i18n( $intervalCount ) );
		} elseif ( $interval === "day" ) {
			$intervalLabel = sprintf(
                /* translators: Singular and plural daily interval label
                 * p1: interval count
                 */
			    _n( 'day', '%d days', $intervalCount, 'wp-full-stripe-admin' ), number_format_i18n( $intervalCount ) );
		}
	...
	protected function updateFormEmailTemplates( $formType, $emailTemplatesJson, & $model ) {
	    $emailTemplates = MM_WPFS_Mailer::extractEmailTemplates( $formType, $emailTemplatesJson );
    ...
    function saveInlineSaveCardForm() {
		if ( ! MM_WPFS_Utils::isDemoMode() ) {
			try {
				...
				$inlineSaveCardFormModel = new MM_WPFS_Admin_InlineSaveCardFormModel();
    ...
    function saveCheckoutPaymentForm() {
        if ( ! MM_WPFS_Utils::isDemoMode() ) {
            try {
                ...
                $checkoutPaymentFormModel = new MM_WPFS_Admin_CheckoutPaymentFormModel();
    ...
    function saveCheckoutSubscriptionForm() {
        if ( ! MM_WPFS_Utils::isDemoMode() ) {
            try {
                ...
                $chekoutSubscriptionFormModel = new MM_WPFS_Admin_CheckoutSubscriptionFormModel();
                $bindingResult = $chekoutSubscriptionFormModel->bind();

                if ( $bindingResult->hasErrors() ) {
                    $return = MM_WPFS_Utils::generateReturnValueFromBindings( $bindingResult );
                } else {
                    $this->updateCheckoutSubscriptionFormEmailTemplates( $chekoutSubscriptionFormModel );

                    $this->db->updateCheckoutSubscriptionForm( $chekoutSubscriptionFormModel->getId(), $chekoutSubscriptionFormModel->getData() );
                    $redirectUrl = MM_WPFS_Admin_Menu::getAdminUrlBySlug(MM_WPFS_Admin_Menu::SLUG_FORMS );
    ...
    function saveCheckoutDonationForm() {
        if ( ! MM_WPFS_Utils::isDemoMode() ) {
            try {
                $checkoutDonationFormModel = new MM_WPFS_Admin_CheckoutDonationFormModel();
    ...
    function saveCheckoutSaveCardForm() {
        if ( ! MM_WPFS_Utils::isDemoMode() ) {
            try {
                ...
                $checkoutSaveCardFormModel = new MM_WPFS_Admin_CheckoutSaveCardFormModel();
    ...
    private function gatherSubscriptionDetails($id ) : array {
        ...
        $subscriptionDetails = array(
            ...
            'localizedAmount'               => MM_WPFS_Admin::getSubscriptionAmountLabel( $subscription, $stripePlan ),
            'date'                          => date( 'F jS Y H:i', strtotime( $subscription->created ) ),
			...
			try {
                ...
                $return = array(
                    ...
                    'msg'     =>
                        ...
                        __( 'There was an error canceling the subscription: ', 'wp-full-stripe-admin' ) . $e->getMessage()
                );
			}
		} else {
            $return = array(
            ...
            try {
                ...
                $return = array(
                    ...
                    'msg'     =>
                        ...
                        __( 'There was an error canceling the donation: ', 'wp-full-stripe-admin' ) . $e->getMessage()
                );
            }
			...
			try {
                ...
                $return = array(
                    ...
                    'msg'     =>
                        ...
                        __( 'There was an error deleting the subscription: ', 'wp-full-stripe-admin' ) . $e->getMessage()
                );
			}
			...
			try {
                ...
                $return = array(
                    ...
                    'msg'     =>
                        ...
                        __( 'There was an error deleting the payment: ', 'wp-full-stripe-admin' ) . $e->getMessage()
                );
			}
		} else {
            $return = array(
            ...
            try {
                ...
                $return = array(
                    ...
                    'msg'     =>
                        ...
                        __( 'There was an error deleting the donation: ', 'wp-full-stripe-admin' ) . $e->getMessage()
                );
            }
        } else {
            $return = array(
    ...
    private function gatherPaymentDetails($id ) : array {
	    ...
	    $paymentDetails = array(
            ...
            'date'                      => date( 'F jS Y H:i', strtotime( $payment->created ) ),
            'amount'                    => $payment->amount,
            'currency'                  => $payment->currency,
            'localizedAmount'           => MM_WPFS_Currencies::formatAndEscape( $payment->currency, $payment->amount ),
            'customerId'                => $stripeCustomerId,
            'customerUrl'               => $this->buildStripeCustomerUrl( $stripeCustomerId, $liveMode ),
    ...
    private function gatherDonationDetails($id ) : array {
        ...
        $paymentDetails = array(
            ...
            'paymentMethodCssClass'     => $paymentMethodCssClass,
            'paymentMethodTooltip'      => $paymentMethodTooltip,
            'date'                      => date( 'F jS Y H:i', strtotime( $donation->created ) ),
            'amount'                    => $donation->amount,
            'currency'                  => $donation->currency,
            'localizedAmount'           => MM_WPFS_Currencies::formatAndEscape( $donation->currency, $donation->amount ),
            'customerId'                => $stripeCustomerId,
            'customerUrl'               => $this->buildStripeCustomerUrl( $stripeCustomerId, $liveMode ),
			...
			try {
                ...
                $return = array(
                    ...
                    'msg'     =>
                        ...
                        __( 'There was an error refunding the payment: ', 'wp-full-stripe-admin' ) . $e->getMessage()
                );
			}
		} else {
            $return = array(
            ...
            try {
                ...
                $return = array(
                    ...
                    'msg'     =>
                        ...
                        __( 'There was an error refunding the donation: ', 'wp-full-stripe-admin' ) . $e->getMessage()
                );
            }
        } else {
            $return = array(
	...
	private function voidOrRefundPayment($paymentId ) {
		if ( $this->debugLog ) {
			MM_WPFS_Utils::log( 'refund(): CALLED, id=' . print_r( $paymentId, true ) );
		}
		$payment = null;
		...
		if ( isset( $payment ) ) {
			...
			if ( $this->debugLog ) {
				MM_WPFS_Utils::log(
					"refund(): payment_status=$paymentStatus, payment_object_type=$paymentObjectType"
				);
			}
			if (
			...
			try {
                ...
                $return = array(
                    ...
                    'msg'     =>
                        ...
                        __( 'There was an error capturing the payment: ', 'wp-full-stripe-admin' ) . $e->getMessage()
                );
            }
		} else {
            $return = array(
	...
	private function captureChargeAndPaymentIntent($paymentId ) {
		if ( $this->debugLog ) {
			MM_WPFS_Utils::log( 'capture(): CALLED, payment_id=' . print_r( $paymentId, true ) );
		}
		$payment = $this->db->getPayment( $paymentId );
		if ( $this->debugLog ) {
		...
		if ( isset( $payment ) ) {
			...
			$paymentObjectType = MM_WPFS_Utils::getPaymentObjectType( $payment );
			if ( $this->debugLog ) {
				MM_WPFS_Utils::log( "capture(): payment_status=$paymentStatus, payment_object_type=$paymentObjectType" );
			}
			if ( MM_WPFS::PAYMENT_STATUS_AUTHORIZED === $paymentStatus ) {
				if ( MM_WPFS::PAYMENT_OBJECT_TYPE_STRIPE_CHARGE === $paymentObjectType ) {
    ...
    private function gatherSavedCardDetails($id ) : array {
        ...
        $savedCardDetails = array(
            ...
            'paymentMethodCssClass'     => $paymentMethodCssClass,
            'paymentMethodTooltip'      => $paymentMethodTooltip,
            'date'                      => date( 'F jS Y H:i', strtotime( $savedCard->created ) ),
			...
			try {
                ...
                $return = array(
                    ...
                    'msg'     =>
                        ...
                        __( 'There was an error deleting the saved card: ', 'wp-full-stripe-admin' ) . $e->getMessage()
                );
            }
		} else {
            $return = array(
    ...
    protected function saveStripeSettings( MM_WPFS_Admin_StripeAccountModel $stripeAccountModel ) {
        ...
        $options[ MM_WPFS::OPTION_API_TEST_SECRET_KEY ]      = $stripeAccountModel->getTestSecretKey();
        $options[ MM_WPFS::OPTION_API_TEST_PUBLISHABLE_KEY ] = $stripeAccountModel->getTestPublishableKey();
        $options[ MM_WPFS::OPTION_API_LIVE_SECRET_KEY ]      = $stripeAccountModel->getLiveSecretKey();
        $options[ MM_WPFS::OPTION_API_LIVE_PUBLISHABLE_KEY ] = $stripeAccountModel->getLivePublishableKey();
        $options[ MM_WPFS::OPTION_API_MODE ]                 = $stripeAccountModel->getApiMode();
    ...
    public function saveStripeAccount() {
        if ( ! MM_WPFS_Utils::isDemoMode() ) {
            try {
                $stripeAccountModel = new MM_WPFS_Admin_StripeAccountModel();
    ...
    protected function saveMyAccountSettings( MM_WPFS_Admin_MyAccountModel $stripeAccountModel ) {
        ...
        $options[ MM_WPFS::OPTION_CUSTOMER_PORTAL_LET_SUBSCRIBERS_CANCEL_SUBSCRIPTIONS ]        = $stripeAccountModel->getCancelSubscriptions();
        $options[ MM_WPFS::OPTION_CUSTOMER_PORTAL_WHEN_CANCEL_SUBSCRIPTIONS ]                   = $stripeAccountModel->getWhenCancelSubscriptions();
        $options[ MM_WPFS::OPTION_CUSTOMER_PORTAL_LET_SUBSCRIBERS_UPDOWNGRADE_SUBSCRIPTIONS ]   = $stripeAccountModel->getUpdowngradeSubscriptions();
        $options[ MM_WPFS::OPTION_CUSTOMER_PORTAL_SHOW_INVOICES_SECTION ]                       = $stripeAccountModel->getShowInvoices();
    ...
    public function saveMyAccount() {
        if ( ! MM_WPFS_Utils::isDemoMode() ) {
            try {
                $myAccountModel = new MM_WPFS_Admin_MyAccountModel();
    ...
    protected function saveWordpressDashboardSettings( MM_WPFS_Admin_WordpressDashboardModel $wpDashboardModel ) {
        ...
        $options[ MM_WPFS::OPTION_DECIMAL_SEPARATOR_SYMBOL ]                    = $wpDashboardModel->getDecimalSeparator();
        $options[ MM_WPFS::OPTION_SHOW_CURRENCY_SYMBOL_INSTEAD_OF_CODE ]        = $wpDashboardModel->getUseSymbolNotCode();
        $options[ MM_WPFS::OPTION_SHOW_CURRENCY_SIGN_AT_FIRST_POSITION ]        = $wpDashboardModel->getCurrencySymbolAtFirstPosition();
        $options[ MM_WPFS::OPTION_PUT_WHITESPACE_BETWEEN_CURRENCY_AND_AMOUNT ]  = $wpDashboardModel->getPutSpaceBetweenSymbolAndAmount();
    ...
    public function saveWordpressDashboard() {
        if ( ! MM_WPFS_Utils::isDemoMode() ) {
            try {
                $wpDashboardModel = new MM_WPFS_Admin_WordpressDashboardModel();
    ...
    protected function saveSecuritySettings( MM_WPFS_Admin_SecurityModel $securityModel ) {
        ...
        $options[ MM_WPFS::OPTION_SECURE_INLINE_FORMS_WITH_GOOGLE_RE_CAPTCHA ]        = $securityModel->getSecureInlineForms();
        $options[ MM_WPFS::OPTION_SECURE_CHECKOUT_FORMS_WITH_GOOGLE_RE_CAPTCHA ]      = $securityModel->getSecureCheckoutForms();
        $options[ MM_WPFS::OPTION_SECURE_SUBSCRIPTION_UPDATE_WITH_GOOGLE_RE_CAPTCHA ] = $securityModel->getSecureCustomerPortal();
        $options[ MM_WPFS::OPTION_GOOGLE_RE_CAPTCHA_SECRET_KEY ]                      = $securityModel->getReCaptchaSecretKey();
        $options[ MM_WPFS::OPTION_GOOGLE_RE_CAPTCHA_SITE_KEY ]                        = $securityModel->getReCaptchaSiteKey();
    ...
    public function saveSecurity() {
        if ( ! MM_WPFS_Utils::isDemoMode() ) {
            try {
                $securityModel = new MM_WPFS_Admin_SecurityModel();
    ...
    protected function saveEmailOptionsSettings( $optionsModel ) {
        ...
        $options[ MM_WPFS::OPTION_EMAIL_NOTIFICATION_SENDER_ADDRESS ] = $fromAddress;
        $options[ MM_WPFS::OPTION_EMAIL_NOTIFICATION_BCC_ADDRESSES ]  = json_encode( $bccAddresses );
    ...
    public function saveEmailOptions() {
        if ( ! MM_WPFS_Utils::isDemoMode() ) {
            try {
                $optionsModel = new MM_WPFS_Admin_EmailOptionsModel();
    ...
    protected function saveEmailTemplatesSettings( $templatesModel ) {
        ...
        $options[ MM_WPFS::OPTION_EMAIL_TEMPLATES ] = json_encode( $templatesModel->getEmailTemplates() );
    ...
    public function saveEmailTemplates() {
        if ( ! MM_WPFS_Utils::isDemoMode() ) {
            try {
                $templatesModel = new MM_WPFS_Admin_EmailTemplatesModel();
    ...
    protected function saveFormsOptionsSettings( $formsOptionsModel ) {
        ...
        $options[ MM_WPFS::OPTION_FILL_IN_EMAIL_FOR_LOGGED_IN_USERS ] = $formsOptionsModel->getFillInEmail();
    ...
    public function saveFormsOptions() {
        if ( ! MM_WPFS_Utils::isDemoMode() ) {
            try {
                $formsOptionsModel = new MM_WPFS_Admin_FormsOptionsModel();
    ...
    protected function saveFormsAppearanceSettings($formsAppearanceModel ) {
        ...
        $options[ MM_WPFS::OPTION_FORM_CUSTOM_CSS ] = $formsAppearanceModel->getCustomCss();
    ...
    public function saveFormsAppearance() {
        if ( ! MM_WPFS_Utils::isDemoMode() ) {
            try {
                $formsAppearanceModel = new MM_WPFS_Admin_FormsAppearanceModel();
    ...
    public function createForm() {
        if ( ! MM_WPFS_Utils::isDemoMode() ) {
            try {
                $createFormModel = new MM_WPFS_Admin_CreateFormModel();
        ...
        switch ( $interval ) {
            case 'day':
                ...
                $formatStr = _n(
                    'daily',
                    'every %d days',
                    $intervalCount, 'wp-full-stripe-admin'
                );
                break;
     ...
     * @return string
     */
    public static function getSubscriptionAmountLabel($subscription, $stripePlan ) {
        $amountLabel = '';

        if ( !is_null( $stripePlan ) ) {
            $formattedAmount = MM_WPFS_Currencies::formatAndEscape( $stripePlan->currency, $stripePlan->unit_amount );
...
class MM_WPFS_Admin_CreateFormFactory {
    ...
    private function compileInlinePaymentFormData( $displayName, $name ) {
        ...
        $form['customAmount']                       = self::CUSTOM_AMOUNT_SPECIFIED_AMOUNT;
