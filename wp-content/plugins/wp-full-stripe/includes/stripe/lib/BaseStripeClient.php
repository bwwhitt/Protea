...
class BaseStripeClient implements StripeClientInterface
{
    ...
    /** @var \StripeWPFS\Util\RequestOptions */
    private $defaultOpts;
    ...
    public function __construct($config = [])
    {
        if (\is_string($config)) {
            ...
            throw new \StripeWPFS\Exception\InvalidArgumentException('$config must be a string or an array');
        }
        ...
        $this->defaultOpts = \StripeWPFS\Util\RequestOptions::parse([
     ...
     * @param array|\StripeWPFS\Util\RequestOptions $opts the special modifiers of the request
     *
     * @return \StripeWPFS\StripeObject the object returned by Stripe's API
     */
    public function request($method, $path, $params, $opts)
    {
        $opts = $this->defaultOpts->merge($opts, true);
        $baseUrl = $opts->apiBase ?: $this->getApiBase();
        $requestor = new \StripeWPFS\ApiRequestor($this->apiKeyForRequest($opts), $baseUrl);
    ...
    public function requestCollection($method, $path, $params, $opts)
    {
        ...
        if (!($obj instanceof \StripeWPFS\Collection)) {
            ...
            throw new \StripeWPFS\Exception\UnexpectedValueException($msg);
        }
    ...
    /**
     * @param \StripeWPFS\Util\RequestOptions $opts
     *
     * @throws \StripeWPFS\Exception\AuthenticationException
     *
     * @return string
     */
    private function apiKeyForRequest($opts)
    {
        ...
        if (null === $apiKey) {
            ...
            throw new \StripeWPFS\Exception\AuthenticationException($msg);
        }
     ...
     *
     * @throws \StripeWPFS\Exception\InvalidArgumentException
     */
    private function validateConfig($config)
    {
        // api_key
        if (null !== $config['api_key'] && !\is_string($config['api_key'])) {
            throw new \StripeWPFS\Exception\InvalidArgumentException('api_key must be null or a string');
        }
        ...
        if (null !== $config['client_id'] && !\is_string($config['client_id'])) {
            throw new \StripeWPFS\Exception\InvalidArgumentException('client_id must be null or a string');
        }

        // stripe_account
        if (null !== $config['stripe_account'] && !\is_string($config['stripe_account'])) {
            throw new \StripeWPFS\Exception\InvalidArgumentException('stripe_account must be null or a string');
        }

        // stripe_version
        if (null !== $config['stripe_version'] && !\is_string($config['stripe_version'])) {
            throw new \StripeWPFS\Exception\InvalidArgumentException('stripe_version must be null or a string');
        }

        // api_base
        if (!\is_string($config['api_base'])) {
            throw new \StripeWPFS\Exception\InvalidArgumentException('api_base must be a string');
        }

        // connect_base
        if (!\is_string($config['connect_base'])) {
            throw new \StripeWPFS\Exception\InvalidArgumentException('connect_base must be a string');
        }

        // files_base
        if (!\is_string($config['files_base'])) {
            throw new \StripeWPFS\Exception\InvalidArgumentException('files_base must be a string');
        }
        ...
        if (!empty($extraConfigKeys)) {
            ...
            throw new \StripeWPFS\Exception\InvalidArgumentException('Found unknown key(s) in configuration array: ' . $invalidKeys);
        }
    }
}
