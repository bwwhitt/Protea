...
class ApiRequestor
{
    ...
    private static function _telemetryJson($requestTelemetry)
    {
        ...
        StripeWPFS::getLogger()->error('Serializing telemetry payload failed!');
    ...
    {
        $uaString = 'Stripe/v1 PhpBindings/' . StripeWPFS::VERSION;

        $langVersion = \PHP_VERSION;
        $uname_disabled = static::_isDisabled(\ini_get('disable_functions'), 'php_uname');
        $uname = $uname_disabled ? '(disabled)' : \php_uname();

        $appInfo = StripeWPFS::getAppInfo();
        $ua = [
            'bindings_version' => StripeWPFS::VERSION,
            'lang' => 'php',
    ...
    private function _requestRaw($method, $url, $params, $headers)
    {
        ...
        if (!$myApiKey) {
            $myApiKey = StripeWPFS::$apiKey;
        }

        if (!$myApiKey) {
            $msg = 'No API key provided.  (HINT: set your API key using '
              . '"StripeWPFS::setApiKey(<API-KEY>)".  You can generate API keys from '
        ...
        if (StripeWPFS::$apiVersion) {
            $defaultHeaders['Stripe-Version'] = StripeWPFS::$apiVersion;
        }

        if (StripeWPFS::$accountId) {
            $defaultHeaders['Stripe-Account'] = StripeWPFS::$accountId;
        }

        if (StripeWPFS::$enableTelemetry && null !== self::$requestTelemetry) {
