...
namespace StripeWPFS\HttpClient;

use StripeWPFS\Exception;
use StripeWPFS\StripeWPFS;
use StripeWPFS\Util;
...
class CurlClient implements ClientInterface
{
    ...
    /** @var \StripeWPFS\Util\RandomGenerator */
     ...
     * @param null|\StripeWPFS\Util\RandomGenerator $randomGenerator
     */
     ...
     *   <li>\StripeWPFS\Util\CaseInsensitiveArray $rheaders The response headers</li>
    ...
    public function request($method, $absUrl, $headers, $params, $hasFile)
    {
        ...
        if (('post' === $method) && (StripeWPFS::$maxNetworkRetries > 0)) {
        ...
        $opts[\CURLOPT_CAINFO] = StripeWPFS::getCABundlePath();
        if (!StripeWPFS::getVerifySslCerts()) {
     ...
     * @param int $errno
     * @param int $rcode
     * @param array|\StripeWPFS\Util\CaseInsensitiveArray $rheaders
     * @param int $numRetries
     *
     * @return bool
     */
    ...
    {
        if ($numRetries >= StripeWPFS::getMaxNetworkRetries()) {
            return false;
        }
    ...
    private function sleepTime($numRetries, $rheaders)
    {
        ...
        $sleepSeconds = \min(
            StripeWPFS::getInitialNetworkRetryDelay() * 1.0 * 2 ** ($numRetries - 1),
            StripeWPFS::getMaxNetworkRetryDelay()
        );
        ...
        $sleepSeconds = \max(StripeWPFS::getInitialNetworkRetryDelay(), $sleepSeconds);

        // And never sleep less than the time the API asks us to wait, assuming it's a reasonable ask.
        $retryAfter = isset($rheaders['retry-after']) ? (float) ($rheaders['retry-after']) : 0.0;
        if (\floor($retryAfter) === $retryAfter && $retryAfter <= StripeWPFS::getMaxRetryAfter()) {
